package streamblocks.opencl.phase;
...
public class OpenCLBackendPhase implements Phase {
    /**
     ...
     * @param backend
     */
    private void generateAuxiliary(OpenCLBackend backend) {
    ...
    private void generateInstances(OpenCLBackend backend) {
        for (Instance instance : backend.task().getNetwork().getInstances()) {
            GlobalEntityDecl entityDecl = backend.globalnames().entityDecl(instance.getEntityName(), true);
    ...
    private void copyResources(OpenCLBackend backend) {
        try {
            // -- Copy Runtime
    ...
    public CompilationTask execute(CompilationTask task, Context context) throws CompilationException {
        ...
        OpenCLBackend backend = MultiJ.from(OpenCLBackend.class)
                .bind("task").to(task)
