...
public interface Instances {
    ...
    default String evaluateCondition(PortCondition condition) {
        if (condition.isInputCondition()) {
            return String.format("%s$FIFO.get_size() >= %d", condition.getPortName().getName(), condition.N());
        } else {
            return String.format("%s$FIFO.get_free_space() >= %d", condition.getPortName().getName(), condition.N());
        }
    }
