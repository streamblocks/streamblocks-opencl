...
public interface Algebraic {
    ...
    interface Compare {
        ...
        default void definition(ProductType product) {
            ...
            product.getFields().forEach(field -> {
                emitter().emit("if (!%s) return false;", backend().statements().compare(field.getType(), String.format("%s->%s", lhs, field.getName()), field.getType(), String.format("%s->%s", rhs, field.getName())));
            });
        ...
        default void definition(SumType sum) {
            ...
            sum.getVariants().forEach(variant -> {
                ...
                emitter().increaseIndentation();
                variant.getFields().forEach(field -> {
                    emitter().emit("if (!%s) return false;", backend().statements().compare(field.getType(), String.format("%s->data.%s.%s", lhs, utils().mangle(variant.getName()), field.getName()), field.getType(), String.format("%s->data.%s.%s", rhs, utils().mangle(variant.getName()), field.getName())));
                });
                emitter().emit("break;");
                emitter().decreaseIndentation();
                emitter().emit("}");
