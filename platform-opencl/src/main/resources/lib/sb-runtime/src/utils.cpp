...
cl_int ReadSourceFromFile(const char *fileName, char **source, size_t *sourceSize) {
    ...
    if (fp == NULL) {
        ...
        *source = new char[*sourceSize];
        if (*source == NULL) {
            std::cout << "Error: Couldn't allocate " << *sourceSize << " bytes for program source from file "
                      << fileName << std::endl;
...
cl_int GetPlatformAndDeviceVersion(cl_platform_id platformID, opencl_arguments *ocl) {
    ...
    if (errorCode != CL_SUCCESS) {
        std::cout << "Error: clGetPlatformInfo() to get CL_PLATFORM_VERSION length returned "
                  << TranslateErrorCode(errorCode) << std::endl;
        return errorCode;
    }
    ...
    if (errorCode != CL_SUCCESS) {
        std::cout << "Error: clGetplatform_ids() to get CL_PLATFORM_VERSION returned " << TranslateErrorCode(errorCode)
                  << std::endl;
        return errorCode;
    }
    ...
    if (errorCode != CL_SUCCESS) {
        std::cout << "Error: clGetDeviceInfo() to get CL_DEVICE_VERSION length returned "
                  << TranslateErrorCode(errorCode) << std::endl;
        return errorCode;
    }
    ...
    if (errorCode != CL_SUCCESS) {
        std::cout << "Error: clGetDeviceInfo() to get CL_DEVICE_VERSION returned " << TranslateErrorCode(errorCode)
                  << std::endl;
        return errorCode;
    }
    ...
    if (errorCode != CL_SUCCESS) {
        std::cout << "Error: clGetDeviceInfo() to get CL_DEVICE_OPENCL_C_VERSION length returned "
                  << TranslateErrorCode(errorCode) << std::endl;
        return errorCode;
    }
    ...
    if (errorCode != CL_SUCCESS) {
        std::cout << "Error: clGetDeviceInfo() to get CL_DEVICE_OPENCL_C_VERSION returned "
                  << TranslateErrorCode(errorCode) << std::endl;
        return errorCode;
...
cl_int CreateAndBuildProgram(opencl_arguments *ocl, std::string fileName) {
    ...
    if (errorCode != CL_SUCCESS) {
        std::cout << "Error: clBuildProgram() for source program returned " << TranslateErrorCode(errorCode)
                  << std::endl;
...
cl_int ExecuteKernel(opencl_arguments *ocl, int index, cl_event *event) {
    ...
    if (ocl->localWorkSize[0] == NULL) {
        errorCode = clEnqueueNDRangeKernel(ocl->commandQueue, ocl->kernels[index], ocl->work_Dim, NULL,
                                           ocl->globalWorkSize, NULL, 0, NULL, event);
    } else {
        errorCode = clEnqueueNDRangeKernel(ocl->commandQueue, ocl->kernels[index], ocl->work_Dim, NULL,
                                           ocl->globalWorkSize, ocl->localWorkSize, 0, NULL, event);
    }

    if (errorCode != CL_SUCCESS) {
        std::cout << "Error: Failed to run the kernel, clEnqueueNDRangeKernel call returned "
                  << TranslateErrorCode(errorCode) << std::endl;
        //return errorCode;
    }
...
bool printClDevices() {
    ...
    for (cl_uint i = 0; i < numPlatforms; ++i) {
        ...
        if (errorCode != CL_SUCCESS) {
            std::cout << "Error: clGetplatform_ids() to get CL_PLATFORM_NAME returned " << TranslateErrorCode(errorCode)
                      << std::endl;
            return false;
        }
        ...
        if (errorCode != CL_SUCCESS) {
            std::cout << "Error: clGetDeviceIDs() to get platforms returned " << TranslateErrorCode(errorCode)
                      << std::endl;
            return false;
        }
        ...
        for (int j = 0; j < numDevices; j++) {
            ...
            if (errorCode != CL_SUCCESS) {
                std::cout << "Error: clGetDeviceInfo() to get device name length returned "
                          << TranslateErrorCode(errorCode) << std::endl;
                return false;
            }
            ...
            if (errorCode != CL_SUCCESS) {
                std::cout << "Error: clGetDeviceInfo() to get device name returned " << TranslateErrorCode(errorCode)
                          << std::endl;
                return false;
            }
            ...
            if (errorCode != CL_SUCCESS) {
                std::cout << "Error: clGetDeviceInfo() to get device type returned " << TranslateErrorCode(errorCode)
                          << std::endl;
                return false;
            }
            ...
            errorCode = clGetDeviceInfo(devices[j], CL_DEVICE_MAX_COMPUTE_UNITS, sizeof(cl_uint), &max_compute_units,
                                        NULL);
            if (errorCode != CL_SUCCESS) {
                std::cout << "Error: clGetDeviceInfo() to get device max compute units returned "
                          << TranslateErrorCode(errorCode) << std::endl;
                return false;
            }
            ...
            errorCode = clGetDeviceInfo(devices[j], CL_DEVICE_MAX_WORK_GROUP_SIZE, sizeof(size_t), &max_work_group_size,
                                        NULL);
            if (errorCode != CL_SUCCESS) {
                std::cout << "Error: clGetDeviceInfo() to get device max work group size returned "
                          << TranslateErrorCode(errorCode) << std::endl;
                return false;
            }
            ...
            if (errorCode != CL_SUCCESS) {
                std::cout << "Error: clGetDeviceInfo() to get device max global mem returned "
                          << TranslateErrorCode(errorCode) << std::endl;
                return false;
            }
            ...
            errorCode = clGetDeviceInfo(devices[j], CL_DEVICE_GLOBAL_MEM_CACHE_SIZE, sizeof(cl_ulong),
                                        &max_global_mem_cache, NULL);
            if (errorCode != CL_SUCCESS) {
                std::cout << "Error: clGetDeviceInfo() to get device max global mem cache size returned "
                          << TranslateErrorCode(errorCode) << std::endl;
                return false;
            }
            ...
            errorCode = clGetDeviceInfo(devices[j], CL_DEVICE_GLOBAL_MEM_CACHELINE_SIZE, sizeof(cl_uint),
                                        &max_global_mem_cacheline_size, NULL);
            if (errorCode != CL_SUCCESS) {
                std::cout << "Error: clGetDeviceInfo() to get device max global mem cache size returned "
                          << TranslateErrorCode(errorCode) << std::endl;
                return false;
            ...
            if (errorCode != CL_SUCCESS) {
                std::cout << "Error: clGetDeviceInfo() to get device max local mem size returned "
                          << TranslateErrorCode(errorCode) << std::endl;
                return false;
            }
...
bool printDeviceInformation(cl_device_id id) {
    ...
    if (errorCode != CL_SUCCESS) {
        std::cout << "Error: clGetDeviceInfo() to get device name length returned " << TranslateErrorCode(errorCode)
                  << std::endl;
        return false;
    }
    std::vector<char> deviceName(deviceStringLength);
...
cl_int SetupOpenCLByDeviceID(opencl_arguments *ocl, cl_device_id id) {
    ...
    *deviceid = id;
    ...
    if (errorCode != CL_SUCCESS) {
        std::cout << "Error: clGetDeviceInfo() to get platform id returned " << TranslateErrorCode(errorCode)
                  << std::endl;
        return errorCode;
    }
    ...
    if ((errorCode != CL_SUCCESS) || (NULL == ocl->context)) {
        std::cout << "Couldn't create a context, clCreateContext() returned " << TranslateErrorCode(errorCode)
                  << std::endl;
        return errorCode;
    }
...
cl_device_id findDeviceID(int id) {
    ...
    errorCode = clGetPlatformIDs(0, NULL, &numPlatforms);
    if (errorCode != CL_SUCCESS) {
        std::cout << "Error: clGetplatform_ids() to get the number of platforms returned "
                  << TranslateErrorCode(errorCode) << std::endl;
        return NULL;
    }
    ...
    for (int i = 0; i < numPlatforms; i++) {
        ...
        errorCode = clGetDeviceIDs(platforms[i], CL_DEVICE_TYPE_ALL, 0, NULL, &numDevices);
        if (errorCode != CL_SUCCESS) {
            std::cout << "Error: clGetDeviceIDs() to get the number of devices returned "
                      << TranslateErrorCode(errorCode) << std::endl;
            return NULL;
        }
